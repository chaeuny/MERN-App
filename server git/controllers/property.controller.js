...
const createProperty = async (req, res) => {
    try {
        ...
        user.allProperties.push(newPropertyu._id);
...
const updateProperty = async (req, res) => {
    try {
        const { id } = req.params;
        const { title, description, propertyType, location, price, photo } = req.params;
...
const deleteProperty = async (req, res) => {
    try {
        ...
        await session.commitTransaction();
            ...
            res.status(200).json({ message: 'Property delted successfully' });
    } catch (error) {
        res.status(500).json({ message: error.message })
    }
};
